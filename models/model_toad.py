...
class TOAD_fc_mtl_concat(nn.Module):
    def __init__(self, gate = True, size_arg = "big", dropout = False, n_classes = 2):
        ...
        #elf.site_classifier = nn.Linear(size[1]+1, 2)      #(site_classifier): Linear(in_features=513, out_features=2, bias=True)
    ...
    def forward(self, h, return_features=False, attention_only=False):
        A, h = self.attention_net(h)    #2任务时，输出结果为1×2      h size=512×1
        ...
        logits  = self.classifier(M[0].unsqueeze(0))   #M[0].unsqueeze(0):1×513   M[0]:513   1×2
