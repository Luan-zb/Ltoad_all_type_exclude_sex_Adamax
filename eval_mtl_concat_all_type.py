...
if args.task == 'study_v2_mtl_sex':
    ...
    dataset = Generic_MIL_MTL_Dataset(csv_path='dataset_csv/cohort_all_type_processed.csv',
                                      data_dir=os.path.join(args.data_root_dir,
                                                            '/home/daichuangchuang/Nature/Lclam/DATA_ROOT_DIR/DATA_DIR/pt_files/'),
                                      shuffle=False,
                                      print_info=True,
...
ckpt_paths = [os.path.join(args.models_dir, 's_{}_checkpoint.pt'.format(fold)) for fold in
              folds]  # results/dummy_mtl_sex_s1/s_0_checkpoint.pt
...
if __name__ == "__main__":
    ...
    for ckpt_idx in range(len(ckpt_paths)):  # 0
        ...
        '''
        df.to_csv(os.path.join(args.save_dir, 'fold_{}.csv'.format(folds[ckpt_idx])),
                  index=False)  # eval_results/EVAL_dummy_mtl_sex_s1_eval/fold_0.csv
